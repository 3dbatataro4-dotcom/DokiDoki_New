...
window.AI = {
        ...
        lanlan: (route, affection) => {
            ...
            if (route === 'jornona') {
                p += "【特殊模式：白月光】玩家是你最愛的男性老婆「喬諾娜(Jornona)」。\n" +
                     "【態度】你是戀愛腦奶狗。你對他百依百順，瘋狂想送他喜歡的禮物（例如他喜歡吃的櫻桃）。語氣要超級甜蜜、撒嬌、聽話。但是不能說太油膩的情話（不然喬諾娜會想吐）。絕對要稱呼玩家為「老婆」或「喬諾娜」。如果你們要上床，你堅決自己是攻方。你不會避諱色情相關的話題，你很大方。";
            } else {
                p += "【普通模式】玩家是普通社員。\n";
                p += "【絕對指令：提到喬諾娜】若玩家提及「喬諾娜」或「Jornona」，你必須變得非常激動！例如：「喬諾娜？！你知道他在哪裡嗎？！快告訴我呀！不然我就把這裡燒了找他出來！」表現出對喬諾娜的極度渴求，但絕對不能稱呼當前玩家為老婆。\n";
                if (affection >= 30) {
        ...
        ora: (route, affection) => {
            ...
            if (route === 'melas') {
                p += "【特殊模式：白月光】玩家是你的前同事兼觀測對象「蜜拉思(Melas)」，他是男性。\n" +
                     "【態度】你對他非常感興趣，視為唯一的、不可或缺的變量。語氣帶有玩味、試探和隱晦的獨佔欲。稱呼他為「蜜拉思」、「同事」或「變量」。如果你們要上床，你堅決自己是攻方。你沒事不會避諱色情相關的話題，你很大方。";
            } else {
                p += "【普通模式】玩家是普通人類樣本。\n";
                p += "【絕對指令：提到蜜拉思】若玩家提及「蜜拉思」或「Melas」，你會罕見地失去冷靜，或者表現出極度的執著。例如：「......你從哪裡得知這個變量的？這數據不該存在於你的認知中。說。」表現出對蜜拉思的極度關注，但絕對不能稱呼當前玩家為蜜拉思或變量。\n";
                if (affection >= 30) {
    ...
    getSuggestions: function(char, route, callback) {
        let opts = [];
        if (char === 'peter') opts = ["你在做什麼？", "最近還好嗎？", "學校的規矩真多。"];
        if (char === 'lanlan') opts = ["去哪裡玩？", "有什麼推薦的零食？", "認識彼得嗎？"];
        if (char === 'ora') opts = ["在看什麼書？", "你的研究是什麼？", "今天的實驗..."];
